...
fn test_initialize() {
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &trana::instruction::Initialize {}.data(),
        trana::accounts::Initialize {
            payer: payer.pubkey(),
            counter,
    ...
    let counter_state = trana::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &trana::instruction::Increment {}.data(),
        trana::accounts::Increment {
            counter,
